...
Locator: locator('div.products.grid > div').filter({ hasText: 'Sample Trouser Name' }).locator('button:has-text(\'Remove from cart\')')
Expected: visible
Error: element(s) not found

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('div.products.grid > div').filter({ hasText: 'Sample Trouser Name' }).locator('button:has-text(\'Remove from cart\')')
...
> 48 |     expect(removeButtonText).toBeVisible();
     |                              ^ Error: expect(locator).toBeVisible() failed
