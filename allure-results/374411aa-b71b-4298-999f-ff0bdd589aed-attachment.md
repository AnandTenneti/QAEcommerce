...
```ts
  ...
  26 |   await loginPage.login("test@qabrains.com", "Password12");
  27 |   const actualErrors = await loginPage.getErrorMessages();
  28 | 
> 29 |   expect(actualErrors.join(" ")).toContain(errorMessages.invalidPassword);
     |                                  ^ Error: expect(received).toContain(expected) // indexOf
  30 | });
  31 | 
```