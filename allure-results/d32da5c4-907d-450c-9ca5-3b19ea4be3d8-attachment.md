...
```ts
  ...
  96  |         const cartItem = cartPage.getProductsInCart(
  97  |           product.name,
  98  |           product.price,
  99  |         );
  100 |         await expect(cartItem).toHaveCount(1);
  101 |         await expect(cartItem).toBeVisible();
  102 |       }
  103 |       // ✅ Calculate totals ONCE (outside loop)
  104 |       const totals = await cartPage.getAllProductTotals();
  105 |       const actualSum = totals.reduce((a, b) => a + b, 0);
  106 | 
  107 |       const expectedTotal = products.reduce(
  108 |         (sum, p) => sum + parseFloat(p.price.replace("$", "")),
  109 |         0,
  110 |       );
  111 | 
> 112 |       await expect(actualSum).toBeCloseTo(expectedTotal, 2);
      |                               ^ Error: expect(received).toBeCloseTo(expected, precision)
  113 |     });
  114 |     await loggedInPage.waitForTimeout(3000);
  115 |   });
  116 | });
  117 | 
```