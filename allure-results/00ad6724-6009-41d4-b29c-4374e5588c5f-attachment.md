...
Locator:  locator('div.cart-list').locator('div.items-center')
Expected: 2
Received: 6
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('div.cart-list').locator('div.items-center')
    3 × locator resolved to 0 elements
      - unexpected value "0"
    6 × locator resolved to 6 elements
      - unexpected value "6"
...
> 95  |       await expect(cartPage.getProductCountInCart()).toHaveCount(products.length);
      |                                                      ^ Error: expect(locator).toHaveCount(expected) failed
  96  |     });
