...
Locator:  locator('div.cart-list').filter({ has: locator('text=undefined') })
Expected: 2
Received: 0
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('div.cart-list').filter({ has: locator('text=undefined') })
...
> 95  |       await expect(cartPage.getProductsInCart()).toHaveCount(products.length);
      |                                                  ^ Error: expect(locator).toHaveCount(expected) failed
  96  |     });
